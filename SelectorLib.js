...
domElement.prototype.animate = async function (from, to, options) {
  ...
  if (options.cleanUp && !options.repeat) {
    await this.wait(options.duration);
  ...
  if (options.yoyo) {
    ...
    await this.wait(options.yoyoDelay + options.yoyoDuration);
  }
...
domElement.prototype.scrollTo = function (options) {
  ...
  const scroll = (axis) => {
    const element = typeof options[axis] === "string" && options[axis] !== "max" ? document.querySelectorAll(options[axis])[0] : [options[axis]][0];
